...
void *naive_malloc(size_t size)
{
    ...
    /* Align size to the next page boundary, excluding the size of the block header */
    ...
    for (block = free_list; block != NULL; block = block->next)
    {
        if (block->size >= aligned_size)
        {
            ...
            return (char *)block + sizeof(Block);  /* Return pointer after block header */
        }
        prev = block;
    }
    ...
    /* Create a new block and return its usable part */
