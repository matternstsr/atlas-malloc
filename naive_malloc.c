...
void *naive_malloc(size_t requested_size)
{
    ...
    mem_block_t *block_ptr = NULL; // Change to mem_block_t

    if (!initialized)
    {
        memory_heap.initial_block = sbrk(0);
        while (memory_heap.total_size < (adjusted_size + sizeof(mem_block_t))) // Update size here
        {
            sbrk(getpagesize());
        ...
        block_ptr = memory_heap.initial_block; // No need to add 1 here, it already points to the correct location
        initialized = 1;
    ...
    block_ptr = (mem_block_t *)navigate_block((sizeof(mem_block_t) + adjusted_size));
...
mem_block_t *navigate_block(size_t size)
{
    ...
    mem_block_t *current_block; // Change to mem_block_t
